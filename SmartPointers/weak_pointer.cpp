...
// Credits : https://pierrchen.blogspot.com/
...
public:
    Memory(int size) : mSize(size), mData(NULL) {
        ALOGD("KK:        Memory constructor %p ",this);
    }

    virtual ~Memory() {
        ALOGD("KK:        Memory destructor %p", this);
        if (mData)  free(mData);
    }

    virtual void onFirstRef() {
        ALOGD("KK:        onFirstRef on %p",this);
        mData = malloc(mSize);
    }
...
#define L(N)   ALOGD("KK: LINE %d TRIGGER:",N);
// print out the strong counter numbers of the object
#define C(obj) ALOGD("KK:        Count of %p : %d", (void*)obj, obj->getStrongCount());
...
int main()
{
    ...
    {//scope for spm1
        ...
        { // scope for spm2
            ...
            if (spm2 != NULL) {
                L(2)
                ALOGD("KK:        Promotion to sp successfully");
                spm2->size();
    ...
    if (spm3 == NULL) {
        ALOGD("KK:        Promotion Fail. Object already been destructed");
    }
